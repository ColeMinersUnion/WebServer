...
### Optimization
Lastly, to find the optimal set of my three input parameters, threads, file buffer size and network buffer size, I used machine learning to find a point that would minimize average and total request times and maximizes throughput. My implementation can be found in `/Optimization.py`.  This uses a Non-dominated Sorting Generic Algorithm (NSGA2) algorithm to find the optimal point. The optimal data set was found to be [3.77619938e-01 5.00761321e-01 3.19060727e-03], or after some rounding, 4 threads, a minimal file buffer size of 5 bytes and network buffer around 3200 bytes. 
